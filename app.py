...
# Strict System Prompt so the AI NEVER forgets its identity
...
def normalize(text):
    ...
    replacements = {
        "aap": "tum", "aapka": "tumhara", "aapki": "tumhari", 
        "kon": "kaun", "bnaya": "banaya", "bnayaa": "banaya"
    }
...
def get_fixed_answer(user_message):
    ...
    # 2. Name / Identity
    if any(k in q for k in ["tumhara naam", "aapka naam", "who are you", "what is your name", "naam kya hai", "naam batao"]):
        return "Mera naam EVARA AI hai. ✨"

    # 3. Specific People Checks
    if any(k in q for k in ["ishaan ke baare", "ishaan kon", "ishaan kaun", "about ishaan" , "apne developer ke bare mein btao" ,"about your developer"]):
    ...
    if any(k in q for k in ["aditi kon", "aditi kaun", "aditi ke baare", "about aditi"]):
        return "Aditi ek choti si pari si ladki hain jo mere developer ki ek bahut achhi dost hain. 😊 Honestly, woh kaafi mature, understanding aur achhi-hearted ladki hain. God bless her! ❤️✨"

    if any(k in q for k in ["divya kon", "divya kaun", "divya ke baare", "didi ji"]):
    ...
    if any(k in q for k in ["sagar", "ayush", "shaurya", "krishu", "naman", "vishal"]):
    ...
    if any(k in q for k in ["yash", "sonu"]):
...
def chat():
    ...
    try:
        ...
        # 2. GROQ MODELS (Llama, DeepSeek, Mixtral)
