...
def chat():
    ...
    try:
        ...
        # Option A: Groq Models (Llama, DeepSeek)
        if model_choice in ["llama-3.3-70b", "deepseek-r1", "mixtral-8x7b"]:
            ...
            # Map frontend model to actual Groq models
            groq_model_map = {
        ...
        # Option B: Gemini Models
        elif model_choice == "gemini-2.0-flash":
            if not GEMINI_API_KEY:
                return jsonify({"error": "GEMINI_API_KEY is missing on Render settings"}), 500

            genai.configure(api_key=GEMINI_API_KEY)
            model = genai.GenerativeModel("gemini-1.5-flash") # Fixed Gemini model name
            response = model.generate_content(user_message)
            response_text = response.text

        # Option C: OpenRouter Models (GPT-4o, Qwen, Claude)
            ...
            # Fallback if unknown model ID passed
